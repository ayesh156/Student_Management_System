...
if (!empty($_POST["t"])) {
    ...
    $query = "SELECT * FROM `student` INNER JOIN `student_details` ON student.email = student_details.student_email INNER JOIN `gender` ON student.gender_id = gender.id INNER JOIN `city` ON student_details.city_id = city.id WHERE `student`.`email` LIKE '%" . $t . "%' OR `student`.`user_name` LIKE '%" . $t . "%' OR `student_details`.`first_name` LIKE '%" . $t . "%' OR `student_details`.`last_name` LIKE '%" . $t . "%' ";
} else {
