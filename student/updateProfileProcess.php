...
$st_details_rs = Database::search("SELECT * FROM `student` WHERE `email`='".$email."' AND `password`='".$password."'");
$st_details_data = $st_details_rs -> fetch_assoc();

$cur_grade = $st_details_data["grade_id"];
...
if($grade > $cur_grade) {
    ...
    if (!empty($bday)) {
        ...
        Database::iud("UPDATE `student` SET `user_name`='" . $uname . "',`password`='" . $password . "',`gender_id`='" . $gender . "',`grade_id`='" . $grade . "' WHERE `email`='" . $email . "' ");
